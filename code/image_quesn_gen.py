...
for img in data.keys():
	...
	if(len(objs)>=2):
		quesn_list.append((" ".join(str(str(objs[0][2]).split(".")[0]).split("_")),"Choose the type of "+" ".join(str(str(objs[1][2]).split(".")[0]).split("_")) +" present in the image?"))
	if(len(objs)>=3):
		quesn_list.append((" ".join(str(str(objs[0][2]).split(".")[0]).split("_")),"What type of "+" ".join(str(str(objs[2][2]).split(".")[0]).split("_"))+" is not shown in the image?" ))
