	...
	def train(self, inputs, targets, num_epochs, vocab_to_ix, first_word_to_ix, model_q):
        ...
        for i in xrange(num_epochs):
            ...
            for j in xrange(len(inputs)):
                ...
                dec_hprev, dec_ps = self.decoder.BPTT([model_q[word] for word in targets[j].split(' ')[:-2]], [vocab_to_ix[word] for word in targets[j].split(' ')[1:]] , enc_hprev, True)
                #Decoder Back pass
                dec_loss, dec_dWxh, dec_dWhh, dec_dWhy, dec_dbh, dec_dby, dec_hprev = self.decoder.BPTT([model_q[word] for word in targets[j].split(' ')[:-2]], [vocab_to_ix[word] for word in targets[j].split(' ')[1:]], dec_hprev)
                ...
                #Encoder Back pass
                enc_loss, enc_dWxh, enc_dWhh, enc_dWhy, enc_dbh, enc_dby, enc_hprev = self.encoder.BPTT(inputs[j], first_word_to_ix[targets[j].split(' ')[0]], enc_hprev)
